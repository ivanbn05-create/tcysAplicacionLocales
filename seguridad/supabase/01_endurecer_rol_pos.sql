...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        SELECT 1
         ...
         WHERE roleid = rol_pos OR member = rol_pos
    ) THEN
        RAISE EXCEPTION
        ...
        SELECT 1
         ...
         CROSS JOIN tipos
          LEFT JOIN pg_default_acl defaults
            ON defaults.defaclrole = propietarios.oid
           AND defaults.defaclnamespace = 'public'::regnamespace
           AND defaults.defaclobjtype = tipos.tipo
         CROSS JOIN LATERAL aclexplode(
             COALESCE(defaults.defaclacl, acldefault(tipos.tipo, propietarios.oid))
         ) permiso
          LEFT JOIN pg_roles receptor ON receptor.oid = permiso.grantee
         WHERE permiso.grantee = 0
