...
1. La clasificación sólo lectura de Arboledas identificó 47 `catalogo.Producto` activos/preciados como menú, 228 registros POS de `legado_incompleto` inactivos/sin precio y, por separado, 38 `ventas.ProductoSucursal` activos con 372 precios mayoristas. Preparar import Central sólo del menú vigente validado; no importar automáticamente los 228 ni los mayoristas. Conservar UUID estables, PB/PL/P4/PK como datos, fotos y excepciones; resolver colisiones sin empatar por nombre y aprobar el acta antes de publicar.
