...
2. **ACK tras restore.** POS `03bab6b` ya recupera la cadena v3 por GET histórico y reproduce mapeos de altas v3 con UUIDv5. Central `570719a` corrigió el `409 ack_state_conflict` para un nuevo `ack_id` sólo cuando Edge/sucursal, publicación, checksum y mapeos exhaustivos coinciden exactamente; diferencias siguen en 409. Falta E2E HTTP/TLS/PostgreSQL del par nuevo; no tratar el menú recuperado como restore integral certificado.
