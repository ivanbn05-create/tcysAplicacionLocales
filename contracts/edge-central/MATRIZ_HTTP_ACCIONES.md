...
Para un ACK v2 de más de 16 KiB y hasta 1 MiB, el Edge consulta `OPTIONS` autenticado a su ruta de acuse. Sólo `204` sin cuerpo, `Cache-Control: no-store`, `X-Catalog-Ack-Profile: mappings-large-1` y `X-Catalog-Ack-Max-Body-Bytes: 1048576` permiten `POST` con el encabezado del perfil. `405`, capacidad ausente/mal formada, `503`, TLS o proxy incompatible conservan `PENDIENTE` con backoff; `401/403` suspenden credencial. Un `413` tras el anuncio deja pendiente el evento válido para revisar despliegue/proxy. Un 200/201 cuyo ACK no corresponde al `ack_id` local exige `CONCILIACION`. Sólo un payload/hash/ruta local inválidos o >1 MiB justifican `CUARENTENA`. El perfil no reescribe JSON, `ack_id`, hash ni historial v2. Si se revierte Central o se apaga su flag tras aceptar un ACK grande cuya respuesta se perdió, Edge conserva el evento pendiente: reactivar el perfil o conciliar contra el registro remoto, sin regenerar UUID ni borrar el outbox. Drenar estos ACK antes de rollback operativo.
