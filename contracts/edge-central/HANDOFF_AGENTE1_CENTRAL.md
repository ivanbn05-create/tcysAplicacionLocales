...
Agente1 confirmó para laboratorio las rutas v3 GET /actual/, GET /{uuid}/ y POST /{uuid}/acuse/, scopes catalog:v3:read/catalog:v3:ack ligados a Edge/sucursal y ACK v3 con el mismo cuerpo/respuesta que v2 salvo version_contrato=3. El token catálogo transitorio puede portar los cuatro scopes v2+v3 para drenar ACK v2 pendientes sin reescribirlos. Límite de ACK v3: 1 MiB. El Edge candidato fue adaptado en la rama codex/production-1.0-catalogo-promociones-dev.2; v2 y v3 permanecen apagados por defecto. Agente1 debe ejecutar E2E PostgreSQL sintético, entregar fixture Central validado contra el schema Edge, probar checksum/ACK y dos sucursales sin filtración cruzada, y después preparar acta de import/reconciliación del catálogo real vigente de Arboledas. La venta de un Edge nuevo permanece bloqueada hasta catálogo v3 completo y alistamiento explícito; una publicación parcial o corrupta no reemplaza la última válida. No hacer deploy/corte desde esta adenda.

Actualización 2026-09-25: agente1 informó Central candidato local `bd0e0b8` y E2E sintético repetido con POS `231513a`: Santa Anita limpia con v3/1→v3/2, PB/PL/P4/PK, precio histórico $35→$37 y ACK aceptados; también v2/1→v2/2→v3/1 con UUID y rutas ACK preservadas. Central reportó 147 pruebas propias y 35 pruebas PostgreSQL enfocadas. Esta evidencia no equivale a importar el catálogo real de Arboledas ni a un corte HTTP/TLS productivo; ambos siguen pendientes.
